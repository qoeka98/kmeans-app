...
def register_custom_font():
    ...
    if os.path.exists(font_path):
        fm.fontManager.addfont(font_path)  # Matplotlib 폰트 등록
...
def main():
    ...
    if file is not None:
        ...
        # 📌 WCSS 그래프 출력 (한글 폰트 적용)
        fig1, ax = plt.subplots()
        ax.plot(range(1, max_k + 1), wcss, marker='o', linestyle='--', color='b')
        ax.set_xlabel('클러스터 개수 (k)', fontsize=12, fontweight='bold', family='NanumSquareRound')
        ax.set_ylabel('WCSS 값', fontsize=12, fontweight='bold', family='NanumSquareRound')
        ax.set_title('엘보우 메서드', fontsize=14, fontweight='bold', family='NanumSquareRound')
